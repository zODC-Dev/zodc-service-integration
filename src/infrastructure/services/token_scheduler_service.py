...
class TokenSchedulerService(ITokenSchedulerService):
        ...
        try:
            ...
            if jira_token:
                if jira_token.expires_at - now <= self.refresh_threshold:
                    await self.refresh_token_chain(user_id, TokenType.JIRA)
                elif not jira_token.is_revoked and jira_token.expires_at - now <= self.refresh_token_threshold:
                    await self.rotate_refresh_token(user_id, TokenType.JIRA)
            ...
            if microsoft_token:
                if microsoft_token.expires_at - now <= self.refresh_threshold:
                    await self.refresh_token_chain(user_id, TokenType.MICROSOFT)
                elif not microsoft_token.is_revoked and microsoft_token.expires_at - now <= self.refresh_token_threshold:
                    await self.rotate_refresh_token(user_id, TokenType.MICROSOFT)
    ...
    async def rotate_refresh_token(self, user_id: int, token_type: TokenType) -> None:
        ...
        try:
            ...
            await self.refresh_token_chain(user_id, token_type)

            # Revoke old refresh token
            await self.refresh_token_repository.revoke_token(current_token.token)
